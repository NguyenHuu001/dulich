...
function Header() {
    ...
    const [tour, setTour] = useState([]); // State cho mảng Tour
    const [lo, setLo] = useState([]); // State cho biến lo
    ...
    useEffect(() => {
        const storedTour = JSON.parse(localStorage.getItem('Tour'));
        setTour(storedTour || []); // Gán vào state tour
        setLo(storedTour || []);   // Gán vào state lo
    }, []);
    useEffect(() => {
        setLo(tour);
    }, [tour]);
