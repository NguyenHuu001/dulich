...
function ChiTietTour(ID) {
    const lls = JSON.parse(localStorage.getItem('Tour')) || [];
    const token1 = localStorage.getItem('token');
    const [parsedTour, setParsedTour] = useState(lls);
    ...
    //     if (fullNamme && phone && date && guest) {
    //         if (token1) {
    //             if (parsedTour) {
    //                 const newTourStorage = [...parsedTour, itemTour];
    //                 localStorage.setItem('Tour', JSON.stringify(newTourStorage));
    //                 // setParsedTour(newTourStorage);
    ...
    const BookTour = async () => {
        ...
        const CartData = {
            userId: '64d8deb62ff3c86a7fef7e0d',
            userEmail: 'lpcuong27@gmail.com',
            tourName: 'DocLaBinhDuong, VietNam',
            fullName: 'asdasd',
            guestSize: 3,
            phone: 5799223,
        };
        ...
        try {
            const response = await CreateItemCart(CartData, con)
        } catch (error) {
