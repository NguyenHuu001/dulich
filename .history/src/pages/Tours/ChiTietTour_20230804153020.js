...
function ChiTietTour() {
    ...
    const getItemReviews = async () => {
        let res = await itemTour.reviews;
        if (res) {
            setItemReviews(res);
        }
    };
    const getItemTour = async () => {
        // Kiểm tra xem dữ liệu đã được lưu trong localStorage hay chưa
        const storedItemTour = localStorage.getItem('itemTour');
        if (storedItemTour) {
            setItemTour(JSON.parse(storedItemTour));
        } else {
            // Nếu chưa có dữ liệu, gọi API để lấy dữ liệu
            let res = await fetchItemTour();
            if (res && res.data) {
                setItemTour(res.data);
                // Lưu dữ liệu vào localStorage
                localStorage.setItem('itemTour', JSON.stringify(res.data));
            }
        }
    };
    return (
                                    ...
                                    <div className="mt-5 d-flex" key={item._id}>
                                        ...
                                        <div className="d-flex ms-4 align-items-center justify-content-between wrap_rating_content">
                                            <div className="d-flex flex-column">
                                                ...
                                                <span className="mt-1">{item.createdAt}</span>
