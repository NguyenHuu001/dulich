...
function Tours() {
    ...
    //Edit tour
    const EditTour = ((item) => {
        const itemEditTour = localStorage.setItem('tourEdit', JSON.stringify(item)
        console.log(itemEditTour);
    })
    //DELE tour
    const DeleteTour = async (_id) => {
        ...
        if (_id) {
            try {
                const del = await DeleteItemTour(_id, config);
                getAllTour(currentPage);
                notification.open({
                    type: 'success',
                    message: 'Thành công',
