...
function ChiTietTour(ID) {
    const lls = localStorage.getItem('Tour');
    const token1 = localStorage.getItem('token');
    const [parsedTour, setParsedTour] = useState(JSON.parse(lls) || []);
    ...
    const BookTour = (itemTour) => {
        if (fullNamme && phone && date && guest) {
            ...
            console.log(fullNamme, phone, date, guest);
            return 0;
        }
    };
